...
- `PosgtreSQL`: Запускает PostgreSQL с помощью bash-команды, указанной в переменной POSTGRESQL_BASH_COMMAND.
- `Redis`: Запускает Redis с помощью bash-команды, указанной в переменной REDIS_BASH_COMMAND.
- `Milvus`: Запускает Milvus с помощью bash-команды, указанной в переменной MILVUS_BASH_COMMAND.
- `Superset`: Запускает Superset с помощью bash-команды, указанной в переменной SUPERSET_BASH_COMMAND.
- `TelegramBot`: Запускает Telegram бота с помощью bash-команды, указанной в переменной TELEGRAM_BASH_COMMAND
- `Model`: Запускает LLM модель с помощью bash-команды, указанной в переменной LLM_BASH_COMMAND.
- `postgres_sensor`: Таска периодически проверяет, запустился ли сервер с БД PostgreSQL, и завершается успешно, если сервер доступен.
- `redis_sensor`: Таска периодически проверяет, запустился ли сервер с БД Redis, и завершается успешно, если сервер доступен.
- `milvus_sensor`: Таска периодически проверяет, запустился ли сервер с БД Milvus, и завершается успешно, если сервер доступен.
- `superset_sensor`: Таска периодически проверяет, запустился ли сервер Superset, и завершается успешно, если сервер доступен.
- `telegram_bot_sensor`: Таска периодически проверяет, запустился ли Telegram бот, и завершается успешно, если сервер доступен.
- `llm_sensor`: Таска периодически проверяет, запустился ли сервер с LLM моделью, и завершается успешно, если сервер доступен.
...
- `Preprocessing`, `Load_to_Milvus`: Таски, отвечающие за предобработку, получение эмбеддингов и загрузку документов в Milvus.

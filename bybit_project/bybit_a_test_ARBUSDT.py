...
def increased_volume_plus_trend(data):
    ...
    elif data[5] > avg_volume_last_hour*1.5:

        if data[4]-data[1] > 0 and trend == 1 and active_order_2 == 0:
...
def several_same_high_prices(klines_data):
    ...
    if (klines_data[1][2] == klines_data[2][2]) and (active_sell_order_1 == False) and (klines_data[1][2], klines_data[2][2]) not in sell_temporary_data_1:
...
def several_same_low_prices(klines_data):
    ...
    if (klines_data[1][3] == klines_data[2][3]) and (active_buy_order_1 == False) and (klines_data[1][3], klines_data[2][3]) not in buy_temporary_data_1:
...
def absorption_of_prev_kline(data_klines):
    ...
    if (abs(float(data_klines[1][4])-float(data_klines[1][1])) > abs(float(data_klines[2][2])-float(data_klines[2][3]))*1.25) and (

...
from first_strategy import new_several_same_high_prices, new_several_same_low_prices
from second_strategy import increased_volume_plus_trend
from third_strategy import new_absorption_of_prev_kline
from vars import trend_flg, avg_volume_last_hour, volume_condition_2
...
def main():
    ...
    while True:
        ...
        new_several_same_high_prices(klines_data[:3])
        new_several_same_low_prices(klines_data[:3])

        # SECOND STRATEGY. INCREASED VOLUME + TREND
        increased_volume_plus_trend(volume_condition_2, klines_data[0])

        # THIRD STRATEGY. ABSORPTION OF SEVERAL KLINES
        new_absorption_of_prev_kline(klines_data[:3])
